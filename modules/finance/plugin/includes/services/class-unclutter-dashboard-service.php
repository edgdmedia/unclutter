...
class Unclutter_Dashboard_Service {
    public static function get_summary($params = []) {
        ...
        if (class_exists('Unclutter_Account_Model')) {
            // Get paginated data, but dashboard usually only needs a few or all
            // We might want to adjust args passed here later if needed (e.g., per_page=-1 for all)
            $accounts_result = Unclutter_Account_Model::get_accounts_by_profile($profile_id);
            // Extract the items array
            if (isset($accounts_result['items'])) {
                $accounts_data = $accounts_result['items'];
            }
        }
        ...
        return [
            ...
            'accounts' => $accounts_data, // Use the extracted items array
        ];
    }
