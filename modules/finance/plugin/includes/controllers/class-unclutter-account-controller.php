...
if ($type_id_param !== null && $type_id_param !== '') {
    $args['type_id'] = intval($type_id_param);
        ...
        if ($request->get_param('page')) {
            $args['page'] = intval($request->get_param('page'));
        }

        // Add per_page filter if provided
        if ($request->get_param('per_page')) {
            $args['per_page'] = intval($request->get_param('per_page'));
        }
        ...
        if ($request->get_param('order_by')) {
            $args['order_by'] = $request->get_param('order_by');
        }
    ...
    public static function get_account($request)
    {
        ...
        return new WP_REST_Response(['success' => true, 'data' => $account], 200);
    }
    ...
    public static function create_account($request)
    {
        ...
        $account = Unclutter_Account_Service::get_account($profile_id, $account_id);
    ...
    public static function get_balance_history($request)
    {
        ...
        // Get balance history
        $history = Unclutter_Account_Service::get_balance_history($profile_id, $id, $start_date, $end_date);
