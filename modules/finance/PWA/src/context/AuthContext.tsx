...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  useEffect(() => {
    console.log('[AuthContext] Initial load: Checking localStorage for token...');
    const storedToken = localStorage.getItem('token');
    if (storedToken) {
      console.log('[AuthContext] Initial load: Found token in localStorage.', storedToken);
      setToken(storedToken);
      setIsAuthenticated(true);
      console.log('[AuthContext] Initial load: Set isAuthenticated = true');
  ...
  const login = async (email: string, password: string) => {
    try {
      ...
      setToken(null);
      setUser(null);
      console.log('[AuthContext] Login error: Set isAuthenticated = false');
  ...
  const logout = async () => {
    try {
      ...
      // Clear state
      console.log('[AuthContext] Logout: Clearing state...');
      setToken(null);
      setUser(null);
      setIsAuthenticated(false);
      console.log('[AuthContext] Logout: Set isAuthenticated = false');
