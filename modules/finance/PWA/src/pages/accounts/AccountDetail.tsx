...
const AccountDetail: React.FC = () => {
  ...
  const [isDeleted, setIsDeleted] = useState(false); // Add state for deletion status
  ...
  const {
    ...
    updateAccount,
    deleteAccount
  } = useAccounts(); // Use useAccounts

  const {
    transactions, // Get transactions from here
    fetchTransactionsByAccount,
    addTransaction,
    isLoadingAccountTransactions // Check this loading state
  } = useTransactions(); // Use useTransactions

  // Find the account in the accounts list
  const account = accounts ? accounts.find(acc => acc.id === id) : null; // Add safety check
  ...
  // Effect to navigate away after successful deletion
  useEffect(() => {
    if (isDeleted) {
      navigate('/accounts');
    }
