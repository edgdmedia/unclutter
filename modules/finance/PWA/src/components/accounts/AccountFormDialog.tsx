...
const AccountFormDialog: React.FC<AccountFormDialogProps> = ({
  ...
  const { categories, fetchCategories } = useCategories(); // Use useCategories
  const accountTypes = categories ? categories.filter(cat => cat.type === 'account_type') : []; // Add safety check
